...
import { VaultDetailLayout } from '@/app/vaults/[chainId]/[address]/vault-detail-layout';
...
export default async function CfoVaultDetailLayout({ children, params }: LayoutProps) {
  const { chainId: chainIdParam, address: addressParam } = await params;

  const chainId = parseInt(chainIdParam, 10);
  if (isNaN(chainId) || !isValidChainId(chainId)) notFound();
  ...
  if (vault && config.id !== 'all' && !vault.apps.includes(config.id as AppId)) notFound();

  const themeClass =
    config.id === 'all' && vault ? getThemeClassForVaultApps(vault.apps) : config.themeClass;
