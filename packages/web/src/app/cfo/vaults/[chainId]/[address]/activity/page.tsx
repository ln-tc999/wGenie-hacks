...
export default async function CfoVaultActivityPage({ params, searchParams }: PageProps) {
  ...
  const activityParams: ActivitySearchParams = {
    chains: chainIdParam, vaults: vaultAddress,
    type: search.type, min_amount: search.min_amount, depositor: search.depositor,
  };
  ...
  try {
    ...
    return (
      <div className="rounded-lg border bg-card p-8 text-center">
        <p className="text-muted-foreground">Unable to load activity data. Please try again.</p>
      </div>
    );
  }
}
